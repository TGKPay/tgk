...
*Setup instructions for a Gitian build of TGK Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TGK
...
- [Building TGK Core](#building-tgk-core)
...
Clone the git repositories for tgk and Gitian.
...
in the tgk repository under 'Fetch and create inputs' to install sources which require
...
To build TGK Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the tgk repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tgk/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/tgk-project/tgk
...
For example:
```bash
URL=https://github.com/thrasher-/tgk.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit tgk=${COMMIT} --url tgk=${URL} ../tgk/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tgk=${COMMIT} --url tgk=${URL} ../tgk/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tgk=${COMMIT} --url tgk=${URL} ../tgk/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the tgk git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tgk/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/tgk
SIGPATH=/some/root/path/tgk-detached-sigs

./bin/gbuild --url tgk=${BTCPATH},signature=${SIGPATH} ../tgk/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tgk-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tgk-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tgk-osx-build.assert
```
...
[tgk-project/gitian.sigs.ltc](https://github.com/tgk-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
