TGK Core version *0.15.1* is now available from:

  <https://download.tgk.org/tgk-0.15.1/>
...
A number of changes to the way TGK Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 TGK). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 TGK) in the next release.
